...
<script type="text/javascript">
        ...
        <div class="col-xs-12 col-sm-8">
            <div class="checkbox">
                ...
                <label for="power_id-<?php echo $key; ?>">
                    <input id="power_id-<?php echo $key; ?>" name="row[power_id][]" type="checkbox" value="<?php echo $vo['id']; ?>" <?php if(is_array($goodsPower) || $goodsPower instanceof \think\Collection || $goodsPower instanceof \think\Paginator): if( count($goodsPower)==0 ) : echo "" ;else: foreach($goodsPower as $key=>$v): if(in_array(($v), is_array($vo['id'])?$vo['id']:explode(',',$vo['id']))): ?>checked<?php endif; endforeach; endif; else: echo "" ;endif; ?>/>
                    <?php echo $vo['title']; ?>
                </label>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8" >
            <textarea id="c-content" style="width:100%; height:200px;"  data-rule="required" placeholder="最多可输入200字" name="row[describe]" type="text" value=""><?php echo $row['describe']; ?></textarea>
        </div>
    </div>
    <div class="form-group">
        <label for="c-price" class="control-label col-xs-12 col-sm-2"><?php echo __('现价'); ?>:</label>
        <div class="col-xs-12 col-sm-8">
            <input id="c-price" data-rule="required" class="form-control" onKeyUp="amount(this)" value="<?php echo $row['price']; ?>" onBlur="overFormat(this)" name="row[price]" type="text" value="">
        </div>
    </div>
    <div class="form-group">
        <label for="c-original_price" class="control-label col-xs-12 col-sm-2"><?php echo __('原价'); ?>:</label>
        <div class="col-xs-12 col-sm-8">
            <input id="c-original_price" data-rule="required" class="form-control" onKeyUp="amount(this)" value="<?php echo $row['original_price']; ?>" onBlur="overFormat(this)" name="row[original_price]" type="text" value="">
        </div>
    </div>
    <div class="form-group">
        <label for="c-duration" class="control-label col-xs-12 col-sm-2"><?php echo __('时限'); ?>:</label>
        <div class="col-xs-12 col-sm-8">
            <input id="c-duration" data-rule="required" placeholder="单位：日" class="form-control" value="<?php echo $row['duration']; ?>"  oninput = "value=value.replace(/[^\d]/g,'')" name="row[duration]" type="text" value="">
        </div>
    </div>
    <div class="form-group">
