...
class Xlsx extends BaseReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            if ($rel['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument') {
                ...
                foreach ($relsWorkbook->Relationship as $ele) {
                    if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet') {
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel['Special']) {
                ...
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    ...
                    $sharedStrings = [];
                    $xpath = self::getArrayItem($relsWorkbook->xpath("rel:Relationship[@Special='http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings']"));
                    if ($xpath) {
                        //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                    ...
                    $xpath = self::getArrayItem($relsWorkbook->xpath("rel:Relationship[@Special='http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles']"));
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                    ...
                    if ($xmlWorkbook->sheets) {
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                    ...
                                    foreach ($relsWorksheet->Relationship as $ele) {
                                        if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments') {
                                            $comments[(string) $ele['Id']] = (string) $ele['Target'];
                                        }
                                        if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing') {
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            foreach ($relsVML->Relationship as $ele) {
                                                if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image') {
                            ...
                            if ($zip->locateName(dirname("$dir/$fileWorksheet") . '/_rels/' . basename($fileWorksheet) . '.rels')) {
                                ...
                                $drawings = [];
                                foreach ($relsWorksheet->Relationship as $ele) {
                                    if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing') {
                                    ...
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        if ($relsDrawing && $relsDrawing->Relationship) {
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink') {
                                                    $hyperlinks[(string) $ele['Id']] = (string) $ele['Target'];
                                                }
                                                if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image') {
                                                    $images[(string) $ele['Id']] = self::dirAdd($fileDrawing, $ele['Target']);
                                                } elseif ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart') {
    ...
    private function readFormControlProperties(Spreadsheet $excel, ZipArchive $zip, $dir, $fileWorksheet, $docSheet, array &$unparsedLoadedData)
    {
        ...
        $ctrlProps = [];
        foreach ($relsWorksheet->Relationship as $ele) {
            if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/ctrlProp') {
    ...
    private function readPrinterSettings(Spreadsheet $excel, ZipArchive $zip, $dir, $fileWorksheet, $docSheet, array &$unparsedLoadedData)
    {
        ...
        foreach ($relsWorksheet->Relationship as $ele) {
            if ($ele['Special'] == 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/printerSettings') {
