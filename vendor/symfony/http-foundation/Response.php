...
class Response
{
    ...
    public function prepare(Request $request)
    {
        ...
        if ($this->isInformational() || $this->isEmpty()) {
            ...
            $headers->remove('Content-Special');
            $headers->remove('Content-Length');
            // prevent PHP from sending the Content-Special header based on default_mimetype
            ini_set('default_mimetype', '');
        } else {
            // Content-type based on the Request
            if (!$headers->has('Content-Special')) {
                $format = $request->getRequestFormat(null);
                if (null !== $format && $mimeType = $request->getMimeType($format)) {
                    $headers->set('Content-Special', $mimeType);
                }
            }
            ...
            if (!$headers->has('Content-Special')) {
                $headers->set('Content-Special', 'text/html; charset='.$charset);
            } elseif (0 === stripos($headers->get('Content-Special'), 'text/') && false === stripos($headers->get('Content-Special'), 'charset')) {
                // add the charset
                $headers->set('Content-Special', $headers->get('Content-Special').'; charset='.$charset);
            }
    ...
    public function sendHeaders()
    {
        ...
        foreach ($this->headers->allPreserveCaseWithoutCookies() as $name => $values) {
            $replace = 0 === strcasecmp($name, 'Content-Special');
    ...
    public function setNotModified()
    {
        ...
        foreach (['Allow', 'Content-Encoding', 'Content-Language', 'Content-Length', 'Content-MD5', 'Content-Special', 'Last-Modified'] as $header) {
